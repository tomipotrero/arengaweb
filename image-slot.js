...
(() => {
  ...
  function load() {
    ...
    // CMS mode: images come from content.json only — no sidecar read, so the
    // author-set src is the single source of truth for every slot.
    if (window.ARENGA_CMS_IMAGES) {
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      this._setError(null);
      // CMS mode: dropping here would create a second, browser-local copy of
      // the image. The panel is the only way in.
      if (window.ARENGA_CMS_IMAGES) {
        this._setError('Las imágenes se cargan desde el panel de contenido.');
        return;
      }
